...
(define/contract (bag-filter-map f xs)
  (parametric->/c (α β) (-> (-> α (or/c #f β)) (listof α) (listof β)))
    ...
    (λ ()
       ...
       (let loop ()
         ...
         (match (thread-receive)
           ['done
            (thread-send parent (cons 'exit id))]
           [(cons 'job x)
            (match (f x)
              [#f (void)]
              [y (thread-send parent (cons 'result y))])
            (loop)]))))
  (define (dispatch worker-ids xs ys)
    (if (empty? worker-ids)
        ys
        (match (thread-receive)
          [(cons 'exit worker-id)
           (dispatch (remove worker-id worker-ids =) xs ys)]
          [(cons 'result y)
           (dispatch worker-ids xs (cons y ys))]
          [(cons 'next worker-thread)
           (match xs
             ['()
              (thread-send worker-thread 'done)
              (dispatch worker-ids xs ys)]
             [(cons x xs)
              (thread-send worker-thread (cons 'job x))
              (dispatch worker-ids xs ys)])])))
  (define workers (range (current-number-of-workers)))
...
(module+ test
  (let* ([f        (λ (x) (if (even? x) x #f))]
         [xs       (range 11)]
         [actual   (sort (bag-filter-map f xs) <)]
         [expected (sort (    filter-map f xs) <)])
    (check-equal? actual expected "bag-filter-map")))
